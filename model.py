...
from GlobalAttention import Att_Attention_Net as Att_Net
...
class CA_NET(nn.Module):
    ...
    def encode(self, atts):
        x = self.relu(self.fc(atts))
        mu = x[:, :self.c_dim]
        logvar = x[:, self.c_dim:]
        return mu, logvar
...
class INIT_STAGE_G(nn.Module):
    def __init__(self, ngf, ncf):
        ...
        self.gf_dim = ngf
        self.in_dim = cfg.GAN.Z_DIM + ncf +312 # cfg.TEXT.EMBEDDING_DIM
    ...
    def forward(self, z_code, c_code, image_atts):
        """
        :param z_code: batch x cfg.GAN.Z_DIM
        :param c_code: batch x cfg.TEXT.EMBEDDING_DIM
        :return: batch x ngf/16 x 64 x 64
        """
        c_z_code = torch.cat((c_code, image_atts, z_code), 1)
...
class INIT_STAGE_G_not_CA(nn.Module):
    def __init__(self, ngf, nef):
        super(INIT_STAGE_G_not_CA, self).__init__()
        self.gf_dim = ngf
        self.in_dim = cfg.GAN.Z_DIM + nef + nef  # cfg.TEXT.EMBEDDING_DIM
    ...
    def forward(self, z_code, atts, image_atts):
        ...
        :return: batch x ngf/16 x 64 x 64
        """
        c_z_code = torch.cat((atts, image_atts, z_code), 1)
        # state size ngf x 4 x 4
        out_code = self.fc(c_z_code)
        out_code = out_code.view(-1, self.gf_dim, 4, 4)
        # state size ngf/3 x 8 x 8
...
class NEXT_STAGE_G(nn.Module):
    ...
    def define_module(self):
        ngf = self.gf_dim
        self.att = Att_Net(ngf)
        self.residual = self._make_layer(ResBlock, ngf*2)
        self.upsample = upBlock(ngf*2, ngf)

    def forward(self, h_code, att_embeddings):
        c_code = self.att(h_code, att_embeddings) ###
...
class G_NET(nn.Module):
    ...
    def forward(self, z_code, atts):
        ...
        """
        fake_imgs = []
        c_code, mu, logvar = self.ca_net(atts)
