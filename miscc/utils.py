    ...
    for i in range(num):
        ...
        for j in range(num_attn):
            ...
            if (vis_size // att_sze) > 1:
                if(vis_size//att_sze==2):
                    one_map = one_map.repeat(2,axis=0).repeat(2,axis=1)
                if(vis_size//att_sze==16):
                    one_map = one_map.repeat(16,axis=0).repeat(16, axis=1)
                #one_map = \
...
def build_images(fake_imgs):
    for i in range(len(fake_imgs)):
        fake_imgs[i] = fake_imgs[i].detach().cpu()
