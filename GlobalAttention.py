...
class Att_Attention_Net(nn.Module):
    ...
    def forward(self, h_code, att_embeddings):
        ...
        ih, iw = h_code.shape[2], h_code.shape[3]
        att_emb = self.att_emb(att_embeddings)
        att_embT = torch.transpose(att_emb, 1,2).contiguous()
        ## att_emb: batch x 312 x ngf
        h_code_flat = h_code.view(batch_size, self.ngf, -1)
        #h_code_flat = torch.transpose(h_code, 1,2).contiguous()
        #attn_map = torch.bmm(h_code_flat, att_embT)
        attn_map = torch.bmm(att_emb, h_code_flat)
        ### batch x K(312) x h*w
        attn_map = nn.Softmax(dim=1)(attn_map)
        weighted_context = torch.bmm(att_embT, attn_map)
        ### batch x ngf x h*w
